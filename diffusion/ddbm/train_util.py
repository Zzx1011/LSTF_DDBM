...
import numpy as np
from LTSF_DDBM.diffusion.ddbm.script_util import NUM_CLASSES

from LTSF_DDBM.diffusion.ddbm.karras_diffusion import karras_sample
...
class TrainLoop:
    ...
    def run_loop(self):
        while True:
            print("train_data length:", len(self.data)) 
            # print("train_data type:", type(self.data))
            ...
            for batch, cond in self.data:
                ...
                batch = batch[:, :1, :, :].repeat(1, 3, 1, 1)
                cond = cond[:, :1, :, :].repeat(1, 3, 1, 1)
                print("batch shape:", batch.shape)
                print("cond shape:", cond.shape)
                padding = (0, 32 - batch.shape[-1])
                batch = F.pad(batch, padding, "constant", 0)
                cond = F.pad(cond, padding, "constant", 0)
                
                # conv_layer = nn.Conv2d(in_channels=32, out_channels=6, kernel_size=(3, 3), stride=1, padding=1).double()
                # batch = conv_layer(batch)
                # cond = conv_layer(cond)
                # print("batch shape after conv:", batch.shape)
                # print("cond shape after conv:", cond.shape)
                ...
                if took_step and self.step % self.save_interval == 0:
                    ...
                    test_batch, test_cond= next(iter(self.test_data))
