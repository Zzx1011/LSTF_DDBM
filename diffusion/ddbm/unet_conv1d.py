...
class AttentionPool1d(nn.Module):
    """
    A 1D version of AttentionPool2d for time series.

    Input shape: (B, C, L)
    Output: (B, output_dim) — the attention pooled feature vector
    """
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            th.randn(embed_dim, seq_len + 1) / embed_dim**0.5
        )
    ...
    def forward(self, x):
        ...
        x = th.cat([x.mean(dim=-1, keepdim=True), x], dim=-1)  # (B, C, L+1)
        x = x + self.positional_embedding[None, :, :].to(x.dtype)  # (B, C, L+1)
        x = self.qkv_proj(x)  # (B, 3C, L+1)
        x = self.attention(x)  # (B, C, L+1)
        x = self.c_proj(x)  # (B, output_dim, L+1)
        return x[:, :, 0]  # 取 CLS token 的位置
...
#     def __init__(self, channels, use_conv, dims=1, out_channels=None):
#         super().__init__()
...
class Upsample(nn.Module):
    def __init__(self, in_channels, use_conv=False, dims=1, out_channels=None):
        super().__init__()
        self.out_channels = out_channels or in_channels
        self.use_conv = use_conv
        if use_conv:
            self.conv = nn.ConvTranspose1d(in_channels, self.out_channels, kernel_size=4, stride=2, padding=1)
        else:
...
class QKVFlashAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"Using FlashAttention with {num_heads} heads")
        print(f"Using FlashAttention with {embed_dim} embed_dim")
...
class UNetConv1DModel(nn.Module):
    """
    ...
    def forward(self, x, timesteps, xT=None, y=None):
        ...
        for module in self.input_blocks:
            # print(f"input_block: h.shape: {h.shape}")
            h = module(h, emb)
            hs.append(h)
